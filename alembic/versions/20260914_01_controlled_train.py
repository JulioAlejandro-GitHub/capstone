"""E9.3 explicit technical revisions and one paused controlled attempt."""
...
def upgrade():
    ...
    for name in ("campaign_technical_guard", "controlled_run_guard", "controlled_binding_guard", "controlled_pause_guard", "campaign_attempt_guard"):
        op.execute(f"ALTER FUNCTION {name}() SET search_path = {schema}, pg_catalog")

def downgrade():
    raise RuntimeError("Forward migration required; preserve technical history")
