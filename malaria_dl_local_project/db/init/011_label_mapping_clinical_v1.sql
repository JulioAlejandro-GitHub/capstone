...
SELECT
    ...
    up.run_metadata,
    COALESCE(
        up.prediction_metadata->>'raw_model_score_meaning',
        up.parameters->>'raw_model_score_meaning',
        up.run_metadata->>'raw_model_score_meaning',
        'probability_parasitized'
    ) AS raw_model_score_meaning,
    COALESCE(
        up.prediction_metadata->>'label_mapping_version',
        up.parameters->>'label_mapping_version',
        up.run_metadata->>'label_mapping_version',
        'clinical_v1_parasitized_positive'
    ) AS label_mapping_version,
    COALESCE(
        up.prediction_metadata #> '{label_mapping}',
        up.parameters #> '{label_mapping}',
        up.run_metadata #> '{label_mapping}',
        jsonb_build_object(
            'version', 'clinical_v1_parasitized_positive',
    ...
    COALESCE(
        up.prediction_metadata->>'positive_class_name',
        up.parameters->>'positive_class_name',
        'parasitized'
    ) AS positive_class_name,
    NULLIF(
        COALESCE(
            up.prediction_metadata->>'positive_class_index',
            up.parameters->>'positive_class_index',
            '1'
        ),
        ''
    )::INTEGER AS positive_class_index,
    COALESCE(
        up.prediction_metadata->>'negative_class_name',
        up.parameters->>'negative_class_name',
        'uninfected'
    ) AS negative_class_name,
    NULLIF(
        COALESCE(
            up.prediction_metadata->>'negative_class_index',
            up.parameters->>'negative_class_index',
            '0'
        ),
        ''
    )::INTEGER AS negative_class_index
