...
def main():
    ...
    if a.operation in ('status','reconcile'):
        ...
        if a.operation=='reconcile' and stored.get('exit_proof'):print(json.dumps(api.call('exit',{**env,'proof':stored['exit_proof']})));return
    ...
    while not stop.is_set():
        ...
        job=api.call('claim',request);stored={'request':request,'job':job};save(stored)
        ...
        child.stdin.write(json.dumps({'job':job,'roots':roots,'url':config['url']}).encode());child.stdin.close()
