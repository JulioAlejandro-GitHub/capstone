...
|R33|Gobierno ML|Existe publicación Stage 2 pero no slot `stage2/default` ejecutable|Catálogo y selección productiva aún desacoplados|Clasificación no puede iniciarse de forma reproducible|Alta|Alto|Crítico|Resolver estricto y estado `awaiting_productive_model`; cero fallback|Publicar/completar el slot desde Modelo IA fuera de Prompt 8|Clasificación|P8|Bloqueado seguro|
|R34|XAI|Grad-CAM puede no encontrar capa o gradientes compatibles|Arquitectura/checkpoint variable|Explicación ausente para una predicción válida|Media|Medio|Medio|Generación manual, estado `unsupported/failed`, predicción independiente|Revisión experta sin XAI|Explainability|P8|Mitigado|
|R35|Ciencia|Fracción de crops positivos interpretada como parasitemia|Agregado experimental visible|Claim clínico indebido|Media|Alto|Crítico|Nombres de candidato, disclaimers y ausencia de campo parasitemia|Ocultar/exportar sólo con revisión futura|Agregación/UI|P8|Mitigado|
|R36|Compatibilidad|Nombre histórico `cell_predictions` era una vista legacy|Nueva tabla especializada requerida|Consumidor SQL legacy falla|Baja|Medio|Medio|Cadena `20260728_01/02/03`: renombrar a `legacy_cell_predictions`, mantener writers en `predictions` y fijar integridad del summary|Vista de compatibilidad versionada futura|DB|P8|Mitigación parcial; cadena final pendiente de gate PostgreSQL|
|R37|Operación|El entorno ML local contiene TensorFlow pero no `PyJWT`, `pwdlib` ni `python-multipart`|Venvs históricos separados y descargas prohibidas en Prompt 8|La API productiva combinada no puede iniciar hasta sincronizar el entorno|Alta|Alto|Crítico|Dependencias declaradas y preflight explícito en startup; no se instaló ni descargó ninguna|Instalación local controlada fuera de este prompt y repetición de la validación|Runtime|P8|Bloqueante|
# Riesgos Prompt 5
