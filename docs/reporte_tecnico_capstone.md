...
- A nivel arquitectónico, el acoplamiento implícito en el ordenamiento de las predicciones binarias TFDS (`0=parasitized`, `1=uninfected`) impone complejidad adicional en el tratamiento de los scores en métricas y visualizaciones.
...
El modelo calcula el `raw_model_score` mediante la activación Sigmoide (salida 1 neurona). Un puntaje cerca de 0 representa la clase "parasitized", lo que el pipeline interpreta invirtiendo/mapeando las probabilidades correctamente mediante `src.decision`.
...
- **Dependencia de la codificación interna estática (0/1):**
  - *Módulo:* Todo el código asume `0 = parasitized` debido a TFDS.
  - *Impacto:* Introduce complejidad aritmética (`1 - score`) que puede acarrear problemas silentes (bugs) ante cualquier cambio sutil en futuros datasets.
  - *Recomendación:* Convertir a multi-output (Softmax de 2 unidades) o hacer una conversión explícita temprana de la etiqueta en el `Dataset.map()`.
